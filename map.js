...
let _formationsLayer = null; // strategic formation clusters (zoom < ZOOM_TACTICAL)
let _zoneCircles = [];       // air/naval zone circles
...
async function initMap(savedView) {
  ...
  _formationsLayer     = L.layerGroup().addTo(_map);
  ...
  _map.on('moveend zoomend', () => {
    if (window.GS) {
      ...
      // Re-render units when crossing the tactical zoom threshold
...
// ── Unit markers — dual-scale (strategic/tactical) ─────────
function renderUnits(state) {
  if (!_map) return;
  _unitsLayer.clearLayers();
  _formationsLayer && _formationsLayer.clearLayers();
  _zoneCircles.forEach(c => _map.removeLayer(c));
  _zoneCircles = [];

  const units   = state.units || [];
  const zoom    = _map.getZoom();

  // Strategic view: formation clusters
  if (typeof clusterUnits === 'function' && zoom < (typeof ZOOM_TACTICAL !== 'undefined' ? ZOOM_TACTICAL : 7)) {
    _renderFormations(units, state);
    return;
  }

  // Tactical view: individual unit markers
  ...
  units.forEach(unit => {
    ...
    if (isPlayer) {
      ...
      marker.on('dragend', (e) => {
        ...
        unit.waypoints.push({ lat, lng });
        if (unit.stance === 'hold' || unit.stance === 'ambush') unit.stance = 'march';
        drawAllPendingRoutes();
        renderPanelForTab('military');
        notify(`${unit.name} — waypoint adicionado (executa no fim do turno)`, 'info');
      });
    }
...
function _renderFormations(units, gs) {
  if (!_formationsLayer || typeof clusterUnits !== 'function') return;
  _formationsLayer.clearLayers();

  const zoom       = _map ? _map.getZoom() : 3;
  const formations = clusterUnits(units, zoom);
  if (!formations) return;

  // Render air/naval zone circles
  if (typeof calcAirSupremacy === 'function') {
    calcAirSupremacy(units, gs).forEach(z => {
      const c = L.circle([z.lat, z.lng], {
        radius: z.radius,
        color:   z.friendly ? 'rgba(58,154,80,0.6)' : z.hostile ? 'rgba(192,37,58,0.5)' : 'rgba(120,120,120,0.3)',
        weight:  1,
        fillColor: z.friendly ? 'rgba(58,154,80,0.06)' : z.hostile ? 'rgba(192,37,58,0.05)' : 'rgba(120,120,120,0.03)',
        fillOpacity: 1,
        interactive: false,
      }).addTo(_formationsLayer);
      _zoneCircles.push(c);
    });
  }

  if (typeof calcNavalZones === 'function') {
    calcNavalZones(units, gs).forEach(z => {
      const c = L.circle([z.lat, z.lng], {
        radius: z.radius,
        color:   z.friendly ? 'rgba(42,106,170,0.6)' : 'rgba(192,37,58,0.4)',
        weight:  1,
        fillColor: z.friendly ? 'rgba(42,106,170,0.06)' : 'rgba(192,37,58,0.04)',
        fillOpacity: 1,
        interactive: false,
      }).addTo(_formationsLayer);
      _zoneCircles.push(c);
  ...
  // Render front lines
  if (typeof calcFrontLines === 'function') {
    calcFrontLines(units, gs).forEach(seg => {
      L.polyline(seg, {
        color: '#c0253a', weight: 2.5, opacity: 0.7, dashArray: '5 4', interactive: false,
      }).addTo(_formationsLayer);
    });
  }

  // Render formation cluster icons
  formations.forEach(f => {
    if (!f.lat || !f.lng) return;
    const html = typeof formationIconHtml === 'function' ? formationIconHtml(f, gs) : `<div>${f.total}</div>`;
    const sz   = Math.min(60, 34 + Math.sqrt(f.total) * 2);
    ...
    const marker = L.marker([f.lat, f.lng], { icon, zIndexOffset: f.country === gs.player_country ? 1000 : 0 });
    marker.on('click', (e) => {
      L.DomEvent.stopPropagation(e);
      // Select the highest-tier unit in the formation
      const top = f.topUnit;
      if (top && top.country === gs.player_country) {
        window._selectedUnitId = top.id;
        notify(`${f.total} unidades de ${f.country} — zoom para ver individualmente`, 'info');
      }
      renderPanelForTab('military');
      switchTab('military');
    });
    marker.addTo(_formationsLayer);
  });
}
...
function _handleMapClickForMove(lat, lng) {
  ...
  // Each click appends a waypoint — unit stays selected for route chaining.
  if (!unit.waypoints) unit.waypoints = [];
  unit.waypoints.push({ lat, lng });
  if (unit.stance === 'hold' || unit.stance === 'ambush') unit.stance = 'march';

  drawAllPendingRoutes();
  const budget = (typeof movementBudgetKm === 'function') ? Math.round(movementBudgetKm(unit)) : null;
  notify(`${unit.name} — waypoint ${unit.waypoints.length} adicionado${budget ? ` (${budget} km/turno)` : ''}. Clique de novo p/ adicionar, ou clique na unidade p/ encerrar.`, 'info');
  renderPanelForTab('military');
}

function clearUnitRoute(unitId) {
  const unit = (window.GS && window.GS.units || []).find(u => u.id === unitId);
  if (!unit) return;
  unit.waypoints = [];
  unit.pendingLat = unit.pendingLng = null;
  drawAllPendingRoutes();
  notify(`${unit.name} — rota cancelada`, 'info');
  renderPanelForTab('military');
}
...
function drawPendingRoute(unit) {
  if (!_map) return;
  const wps = (unit.waypoints && unit.waypoints.length)
    ? unit.waypoints
    : (unit.pendingLat != null ? [{ lat: unit.pendingLat, lng: unit.pendingLng }] : []);
  if (!wps.length) return;

  const lines = [];
  let prevLat = unit.lat, prevLng = unit.lng;
  let lastPts = null;

  wps.forEach((wp, i) => {
    const pts = _bezierPoints(prevLat, prevLng, wp.lat, wp.lng, 20);
    lastPts = pts;
    ...
    // Diamond marker at intermediate waypoints
    if (i < wps.length - 1) {
      const wpIcon = (typeof createMapSpriteIcon === 'function') ? createMapSpriteIcon('map_waypoint_diamond', 18) : null;
      if (wpIcon) {
        const m = L.marker([wp.lat, wp.lng], { icon: wpIcon, interactive: false, zIndexOffset: -90 }).addTo(_map);
        lines.push(m); // cleaned up alongside lines
      }
    }
    prevLat = wp.lat; prevLng = wp.lng;
  });

  // Arrowhead at the final destination
  const n = lastPts.length;
  const dy = lastPts[n-1][0] - lastPts[n-2][0];
  const dx = lastPts[n-1][1] - lastPts[n-2][1];
  ...
  const arrowMarker = L.marker([dest.lat, dest.lng], { icon: arrowIcon, interactive: false, zIndexOffset: -100 }).addTo(_map);
  _pendingRoutes.push({ lines, arrowMarker, unitId: unit.id });
}
...
function drawAllPendingRoutes() {
  ...
  if (!window.GS) return;
  (window.GS.units || [])
    .filter(u => u.country === window.GS.player_country &&
      ((u.waypoints && u.waypoints.length) || u.pendingLat != null))
